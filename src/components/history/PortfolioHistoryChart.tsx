    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-spotify-dark-gray p-4 rounded-lg border border-gray-600 shadow-lg">
          ...
          <p
            className={`${data.gainLoss >= 0 ? 'text-green-400' : 'text-red-400'}`}
          >
            <span className="font-medium">Gain/Loss:</span>{' '}
            {formatCurrency(data.gainLoss)} ({data.gainLossPercent.toFixed(2)}%)
          </p>
        </div>
      );
    }
      ...
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500">
            Add some stocks and take snapshots to see performance over time
          </p>
        </div>
      </div>
    );
