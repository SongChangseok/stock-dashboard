...
const PerformanceMetrics: React.FC<PerformanceMetricsProps> = ({ className = '' }) => {
  ...
  if (!metrics) {
    return (
      <div className={`bg-spotify-gray p-6 rounded-lg border border-gray-700 ${className}`}>
        <div className="text-center py-8">
          <Activity className="w-12 h-12 text-gray-400 mx-auto mb-4" />
          <p className="text-gray-400">No performance data available</p>
          <p className="text-sm text-gray-500 mt-2">Take portfolio snapshots to see performance metrics</p>
        </div>
      </div>
    );
  }
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {metricCards.map((metric, index) => {
          ...
          return (
            <div key={index} className="bg-spotify-gray p-4 rounded-lg border border-gray-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="text-center">
            ...
            <div className="text-xs text-gray-500">
              {metrics.volatility < 15 ? 'Low Risk' : 
               metrics.volatility < 25 ? 'Medium Risk' : 'High Risk'}
            </div>
          </div>
          
          <div className="text-center">
            <div className={`text-2xl font-bold mb-2 ${getColorClass(metrics.sharpeRatio)}`}>
          ...
          <div className="text-center">
            <div className={`text-2xl font-bold mb-2 ${getColorClass(metrics.maxDrawdownPercent, true)}`}>
              {formatPercentage(metrics.maxDrawdownPercent)}
            </div>
            <div className="text-sm text-gray-400 mb-1">Max Drawdown</div>
            <div className="text-xs text-gray-500">
              {Math.abs(metrics.maxDrawdownPercent) < 10 ? 'Low Drawdown' : 
               Math.abs(metrics.maxDrawdownPercent) < 20 ? 'Moderate Drawdown' : 'High Drawdown'}
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <span className="text-green-400 font-semibold">
              {metrics.winRate > 60 ? 'Strong uptrend' : 
               metrics.winRate > 50 ? 'Positive trend' : 'Mixed performance'}
            </span>
          </div>
          
          <div className="flex justify-between items-center">
            <span className="text-gray-300">Risk-Adjusted Performance:</span>
            <span className={`font-semibold ${getColorClass(metrics.sharpeRatio)}`}>
              {metrics.sharpeRatio > 1 ? 'Above average' : 
               metrics.sharpeRatio > 0 ? 'Average' : 'Below average'}
            </span>
          </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <span className={`font-semibold ${
              ...
              {metrics.volatility < 15 ? 'High' : 
               metrics.volatility < 25 ? 'Medium' : 'Low'}
            </span>
          </div>
        </div>
      </div>
