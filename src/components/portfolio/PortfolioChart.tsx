...
const PortfolioChart: React.FC<PortfolioChartProps> = ({ data }) => {
  ...
  const [selectedStock, setSelectedStock] = useState<PortfolioData | null>(null);
  ...
  const handlePieClick = (data: any, index: number) => {
    ...
    setSelectedStock(selectedStock?.name === clickedStock.name ? null : clickedStock);
  };
  ...
  const CenterInfo = () => (
    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
      <div className="text-center">
        {selectedStock ? (
          <>
            <div className="text-sm text-slate-400 mb-1">{selectedStock.name}</div>
  ...
  return (
    <div className="glass-card-dark rounded-2xl p-8 mb-8 animate-fade-in">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold gradient-text-primary">Portfolio Distribution</h2>
        {selectedStock && (
          <button
                ...
                >
                  {data.map((entry, index) => (
                    <Cell 
                      key={`cell-${index}`} 
                      fill={entry.color}
                      fillOpacity={selectedStock && selectedStock.name !== entry.name ? 0.3 : 1}
                    />
                  ))}
                </Pie>
                <Tooltip 
